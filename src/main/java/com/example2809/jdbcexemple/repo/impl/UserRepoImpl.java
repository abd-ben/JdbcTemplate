...
public class UserRepoImpl implements UserRepo {
  ...
  public User findById(Integer id) {
    return jdbc.queryForObject(
        "select id, name, address, phone from User where id=?", this::mapRowToUser, id);
  }

  @Override
  public Iterable<User> findAll() {
    return jdbc.query("select id, name, address, phone from User", this::mapRowToUser);
  }

  private User mapRowToUser(ResultSet rs, int rowNum) throws SQLException {
    return new User(
        rs.getInt("id"), rs.getString("name"), rs.getString("address"), rs.getString("phone"));
  }
}
